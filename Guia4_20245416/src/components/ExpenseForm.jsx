...
export const ExpenseForm = () => {
    ...
    const handleSubmit = (e) => {
        ...
        // ver que no se pase del presupuesto
        const totalExpenses = state.expenses.reduce((total, currentExpense) => currentExpense.amount + total, 0)
        
        let availableBudget = state.budget - totalExpenses

        // si editamos el $ que sume lo anterior 
        if (state.editingId) {
            const previousExpense = state.expenses.find(exp => exp.id === state.editingId)
            availableBudget += previousExpense.amount
        }
